...
class BuilderAgentReport:
    ...
    def to_markdown(self) -> str:
        ...
        if self.package_result is None:
            ...
            lines.append("PASS" if self.package_result.success else "FAIL")
            lines.extend(["", "```text", self.package_result.to_message(), "```"])
    ...
    def _generate_valid_file(
        ...
        for attempt in range(self.retry_limit + 1):
            try:
                ...
                fallback = self._fallback_content(spec, project_file)
                if fallback is None:
                    raise
                self.logger.warning("LLM unavailable; using validated fallback for %s.", project_file.path)
                content = fallback
            ...
            report.retries += 1
        fallback = self._fallback_content(spec, project_file)
        if fallback is not None and self.generation_validator.validate(spec, project_file, fallback).success:
            self.logger.warning("Generated content rejected; using validated fallback for %s.", project_file.path)
            return fallback
        raise ValueError(f"Generated content failed validation for {project_file.path}: {'; '.join(last_errors)}")

    def _fallback_content(self, spec: ProjectSpec, project_file: ProjectFile) -> str | None:
        """Return safe fallback content for explicit standard project files."""
        path = project_file.path.as_posix()
        package = spec.slug
        if path == "README.md":
        ...
        if path == f"{package}/__init__.py":
            if (Path(path).parent / "hello.py").as_posix() in {item.path.as_posix() for item in spec.files}:
                return "from .hello import greet\n\n__all__ = [\"greet\"]\n"
            return '"""Generated package."""\n'
        if path == f"{package}/hello.py" and "hello" in spec.description.lower():
            return "def greet() -> str:\n    \"\"\"Return a Hello World greeting.\"\"\"\n    return \"Hello, World!\"\n"
        if path == "tests/test_hello.py" and "hello" in spec.description.lower():
            return (
                f"from {package}.hello import greet\n\n\n"
                "def test_greet() -> None:\n"
    ...
    def _validate_and_repair(self, spec: ProjectSpec, project_root: Path, report: BuilderAgentReport) -> None:
        for attempt in range(self.retry_limit + 1):
            ...
            if not package_result.success:
                report.errors.append(package_result.to_message())
                return
